...
;	(defconstant txt_rules "bug_rr.txt")
;	(defconstant txt_toks "bug_rs.txt")
;	(defconstant txt_proper "bug_pn.txt")
...
(defun eliza()
	...
	(let  ((all_rules (load_rules)) (all_words (load_words)))
		(loop 
			(print 'eliza>)
			(write  (use_res_rules (read) all_rules all_words)  )
		)
	)
)
			...
			(some #'(lambda (word)
					(let* ((result (pat_match (gen_res_pat word) input))
						...
						(cond 
							...
							(t (use_res_rules_aux result_match rest_rule all_words list_word))
						)
					)
